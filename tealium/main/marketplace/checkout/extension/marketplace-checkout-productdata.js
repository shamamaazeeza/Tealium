 ...
 */
if (utag.data['dom.url'].indexOf('ibm.com/marketplace/cloud/sec/OrderShippingBillingConfirmationMarketplaceView') > -1
      || utag.data['dom.url'].indexOf('ibm.com/marketplace/cloud/OrderShippingBillingConfirmationMarketplaceView') > -1) {
   var docCompleteTimer = setInterval(function() {
      if (document.readyState == 'complete') {
         ...
         utag.data.marketplace_checkout_confirmation = true;
         
         // Call utag.view for only the tag(s) that require waiting until document is complete
         utag.view(utag.data, null, [2059, 2060, 373644, 1175, 373957530]);
      }
   }, 100);
}