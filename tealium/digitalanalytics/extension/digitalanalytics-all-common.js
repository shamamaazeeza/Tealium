...
/*---------------------------------------------------setting Site ID---------------------------------------------------------*/
//set siteID from meta IBM.WTMSite, WTMSite or null string
utag.data.site_id = utag.data.site_id || String(get_meta_tag("IBM.WTMSite")) || String(get_meta_tag("WTMSite")) || "null";
...
if(utag.data.site_id.substring(0, 4).toLowerCase() == "test" || utag.data.site_id.substring(utag.data.site_id.length-4, utag.data.site_id.length).toLowerCase() == "test"){
   ...
   var isClientidSet = false;
   var x = utag.data.test_domain.split(',');
   for(var j=0; j<x.length; j++){
      if (document.domain.toLowerCase() == x[j]) {
         utag.data.concat_clientid = "80200000|" + utag.data.site_id;
         isClientidSet = true;
      }
   }
   if (!isClientidSet) utag.data.concat_clientid = "50200000|" + utag.data.site_id;
}else{
