               ...
               if (data.type !== "product") {
                  if (data.type == "video") {
                     ...
                     if (data.eventVidTimeStamp.toLowerCase() == "start" || data.eventCategoryGroup.toLowerCase() == "start") {
                        ...
                        cmsdk.event("link", dataConversion);
                     }
...
var cmsdk = {
      ...
      shortname_to_ddo_map : {},

      getShortName : function (name) {
         var nameParts = name.split(".");
         for (var i = nameParts.length; i > 0; i--) {
            var testName = nameParts.slice(0, (i)).join('.') + '.';
            if (cmsdk.ddo_to_shortname_map[testName]) {
               return cmsdk.ddo_to_shortname_map[testName] + nameParts.slice(i, nameParts.length).join('.');
            }
         }
         return name;
      },

      getLongName : function (name) {
         var nameParts = name.split(".");
         for (var i = nameParts.length; i > 0; i--) {
            var testName = nameParts.slice(0, (i)).join('.') + '.';
            if (cmsdk.shortname_to_ddo_map[testName]) {
               return cmsdk.shortname_to_ddo_map[testName] + nameParts.slice(i, nameParts.length).join('.');
            }
         }
         return name;
      },
      ...
      ignoreKey : function (key) {
         ...
         Object.keys(cmsdk.ignore_keys).forEach(function (name) {
      ...
      processDataObject : function (obj, new_obj, parent_key, create_array) {
         if (typeof parent_key === "undefined") {
            ...
            parent_key += "" + cmsdk.nested_delimiter;
         }
         Object.keys(obj).forEach(function (key) {
            var nested_key_name = parent_key + key;
            //Format the new key name and take out any whitespace
            var new_key_name = (cmsdk.prefix + parent_key + key).replace(/\s/g, '');
            if (typeof obj[key] !== 'undefined' && obj[key] != null) {
               if ((typeof obj[key]).match(/boolean|string|number/) && !cmsdk.ignoreKey(key)) {
                  //Update new_key_name to the shortened version of the name
                  var temp_parent_key = parent_key.indexOf(cmsdk.prefix) < 0 ? (cmsdk.prefix + parent_key).replace(/\s/g, '') : parent_key.replace(/\s/g, '');
                  new_key_name = cmsdk.getShortName(temp_parent_key) + key.replace(/\s/g, '');
                  // if(cmsdk.ddo_to_shortname_map[temp_parent_key]){
                  //   new_key_name = cmsdk.ddo_to_shortname_map[temp_parent_key]+key.replace(/\s/g, '');
                  // }
                  if (create_array) {
                     //First check to see if this key exists
                     if (cmsdk.typeOf(new_obj[new_key_name]) !== "array") {
               ...
               else if (cmsdk.typeOf(obj[key]) === 'object' && !cmsdk.ignoreKey(key)) {
                  //Process this piece of the data layer and merge it
                  cmsdk.processDataObject(obj[key], new_obj, nested_key_name, create_array);
               }
               else if (cmsdk.typeOf(obj[key]) === 'array') {
                  cmsdk.processDataArray(obj[key], new_obj, nested_key_name);
               }
            }
         });
      },

      processDataArray : function (obj, new_obj, parent_key) {
         if (typeof parent_key === "undefined") {
            //This object isn't nested in another object
            parent_key = "";
         }
         ...
         //Format the new key name and take out any whitespace
         var new_key_name = (cmsdk.prefix + parent_key).replace(/\s/g, '');
         for (var n = 0; n < obj.length; n++) {
            if ((typeof obj[n]).match(/boolean|string|number/)) {
               //Update new_key_name to the shortened version of the name
               new_key_name = cmsdk.getShortName(new_key_name);
               // if(cmsdk.ddo_to_shortname_map[new_key_name]){
               //   new_key_name = cmsdk.ddo_to_shortname_map[new_key_name];
               // }
               //First check to see if this key exists
               if (cmsdk.typeOf(new_obj[new_key_name]) !== "array") {
                  //Make the key an array
            ...
            else if (cmsdk.typeOf(obj[n]) === 'object') {
               Object.keys(obj[n]).forEach(function (array_key) {
                  ...
                  if (typeof temp_obj[array_key] !== 'undefined' && temp_obj[array_key] != null) {
                     if ((typeof temp_obj[array_key]).match(/boolean|string|number/) && !cmsdk.ignoreKey(array_key)) {
                        //Update new_key_name to the shortened version of the name
                        array_key_name = cmsdk.getShortName(new_key_name) + array_key.replace(/\s/g, '');
                        // if(cmsdk.ddo_to_shortname_map[new_key_name]){
                        //   array_key_name = cmsdk.ddo_to_shortname_map[new_key_name]+array_key.replace(/\s/g, '');
                        // }
                        //First check to see if this key exists
                     ...
                     else if (cmsdk.typeOf(temp_obj[array_key]) === 'object') {
                        cmsdk.processDataObject(temp_obj[array_key], new_obj, array_key_name, 1);
                     }
                     else if (cmsdk.typeOf(temp_obj[array_key]) === 'array') {
                        cmsdk.processDataArray(temp_obj[array_key], new_obj, array_key_name);
                     }
                  }
               });
            }
      ...
      set_shortname_to_ddo_map : function () {
         Object.keys(cmsdk.ddo_to_shortname_map).forEach(function (key) {
            cmsdk.shortname_to_ddo_map[cmsdk.ddo_to_shortname_map[key]] = key;
         });
      },
      ...
      event : function (a,b) {
         try {
            ...
            if (typeof(digitalData.page.isDataLayerReady) === "undefined") {
               dl.init(0);
               dl.log('+++DBDM-LOG > event: digitalData was reset, recreating datalayer');
            }
            /* Ensure that we capture the CoreID6 cookie ID */
            if (typeof(digitalData.page.pageInfo.coremetrics.visitorID) === "undefined" && a != "view") {
               dl.fn.readCookies();
               dl.log('+++DBDM-LOG > event: Reading first-party cookies');
            }
            /* Ensure that we capture the anonymous ID from the cookie */
            if (typeof(digitalData.user.profile.auid) === "undefined") {
               digitalData.user.profile.auid = dl.fn.getCookie('BMAID');
            }
            ...
            cmsdk.processDataObject(digitalData, b);
            cm.exec(a, b);
...
var cm = {
      ...
      /*--------------------Set NTPT attributes --------------------*/
      getNTPTVariable : function (addParams) {   
      ...
      /*--------------------Add attributes --------------------*/
      addAttributes : function (clientArray,value) {   
      ...
      getCmCreateProductView : function (n) {
      ...
      initClient : function () {
         try {
            ...
            if(typeof (document.domain) !== 'undefined' && document.domain.indexOf('ibm.com') !== -1) {
               cmTagQueue.push(['cmSetupCookieMigration', true, true, null, dl.DOMAIN_BACKLIST]);
            }
      ...
      prepareAttr : function (prefix, tag, numAttrs, index, cindex, attrStr) {
         attrStr = "";
