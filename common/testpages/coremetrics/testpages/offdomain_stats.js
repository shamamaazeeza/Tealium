/*!
 * $Id: stats.source.js,v 1.118 2011/06/14 13:50:14 jtalekar Exp $
...
var NTPT_MAXTAGWAIT = 1.0, // Max delay (secs) on link-tags and submit-tags
	//alert("hi shib");
	// Optional variables:
	NTPT_HTTPSIMGSRC = '//pt200204.unica.com/ntpagetag.gif',
	...
	NTPT_GLBLCOOKIES = [];
	//var NTPT_GLBLCOOKIES = ["IBMISP","iwm1p"]; 
...
/*function to bind different events*/
...
/*function to check the domain from NTPT_DOMAINLIST*/
    ...
    if (host.length > 0) {
        ...
        if (host == window.location.hostname.toLowerCase()) {
            ...
            var doms = varlist(NTPT_DOMAINLIST),
            	len = doms.length;
            for (var i = 0; i < len; i++) {
                if ((doms[i] !== "") && (host == doms[i] || host.search(doms[i]) != -1)) {
                    bool = true;
                }
            }
        }
...
function check_megamenu_element(el) {
    do {
...
/*function for tracking left click on links*/
function download_offsite_tracking(evt) {
...
/*function for tracking middle click on links*/
function middle_click_tracking(evt) {
    evt = evt || (window.event || "");
...
/*function for tracking right click on links*/
    ...
    if ((typeof e.tagName !== 'undefined' && e.tagName.toLowerCase() == 'a') && !!e.href) {
        ...
        //if (megamenuElement == false) {
            var hn = e.hostname ? (e.hostname.split(":")[0]) : "",
                ...
                qs1 = new Querystring(qry),
                v1 = qs1.get("attachment"),
                v2 = qs1.get("FILE"),
                v3 = qs1.get("attachmentName");

            evLinkTitle = (evLinkTitle == "") ? null : evLinkTitle;
            ...
            if (domtest(hn)) {
                if (match(download_uri, NTPT_DOWNLOADTYPES) || match(download_param, NTPT_DOWNLOADTYPES)) {
                    //function for download tracking
                    var pth = e.pathname ? ((e.pathname.indexOf("/") != 0) ? "/" + e.pathname : e.pathname) : "/",
                        ttl = "",
                        text = document.all ? e.innerText : e.text,
                        img = evt_element(evt, "IMG");
                    if (img.alt) {
                        ttl = img.alt;
                    } else if (text) {
                        ttl = text;
                    ...
                    if (evhndlr != false) {
                        if (vparam == "none") {
                            ntptEventTag('ibmEvAction=' + evAction.toLowerCase() + '&ev=download&ibmEvGroup=null&ibmEvName=' + evLinkTitle + '&ibmEvModule=null&ibmEvSection=null&ibmEvTarget=' + evAction.toLowerCase() + '&ibmEvFileSize=null&ibmEvLinkTitle=' + evLinkTitle);
                            pause(500);
                        } else {
                            ntptEventTag('ibmEvAction=' + download_param + '&ev=download&ibmEvGroup=null&ibmEvName=' + evLinkTitle + '&ibmEvModule=null&ibmEvSection=null&ibmEvTarget=' + evAction.toLowerCase() + '&ibmEvFileSize=null&ibmEvLinkTitle=' + evLinkTitle);
                            pause(500);
                        }
                    }
                    ...
                    ntptEventTag('ibmEvAction=' + evAction.toLowerCase() + '&ev=page click&ibmEvGroup=null&ibmEvName=' + evLinkTitle + '&ibmEvModule=null&ibmEvSection=null&ibmEvTarget=' + evAction.toLowerCase() + '&ibmEvFileSize=null&ibmEvLinkTitle=' + evLinkTitle);
                    pause(500);
                }
            } else {
                ...
                if ((hn.length > 0) && (pr.indexOf("http") == 0 || pr.indexOf("mailto") == 0) && (!domtest(hn))) {
                    if (evhndlr != false) {
                        ntptEventTag('ev=external link&ibmEvAction=' + evAction.toLowerCase() + '&ibmEvGroup=null&ibmEvName=' + evLinkTitle + '&ibmEvModule=null&ibmEvSection=null&ibmEvTarget=' + evAction.toLowerCase() + '&ibmEvFileSize=null&ibmEvLinkTitle=' + evLinkTitle);
                        pause(500);
                    }
...
(function () {
    ...
    var checkAndAdd = function (name) {
        var c = getMT(name);
        if (name == 'IBM.Country') {
            NTPT_GLBLEXTRA = 'ibm.inputcountry=' + c;
        }
        if (!!c) ntptAddPair(name, c);
    };

    /*ntptAddPair("site", "ibmcom");*/
    //ntptAddPair("site", "SupplierConnection");
    ntptAddPair("ibm.wtmsite", "ibmoffdomain");
	...
	**/
    ...
    if (!!window.ibmCommonErrorpagesStatus) {
        ntptAddPair("sc", window.ibmCommonErrorpagesStatus)
    }
...
**/
function NETINSIGHT_OVERLAY_NORMALIZE_URL_CALLBACK(url) {
